...
### transfer\(\)
Transfers `token` from the `from` address to the `to` address. Will invoke `NFTReceiver` if `to` is a contract. If provided `data` will be submitted with the invocation of `NFTReceiver`. Emits the `Transfer` event.
...
contract interface NFTReceiver = 
    entrypoint onNFTReceived : (address, address, int, option(string)) => bool
```
...
Returns `true` or `false` to signal whether processing the received token was successful or not.
